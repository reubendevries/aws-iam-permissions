...
def read_permission_documents():
    ...
    with open("./aws-iam-permissions/create_group.json") as group_doc:
        group = json.loads(group_doc)
        return group
    
def create_iam_group(group):
    """ a function that will create a new iam policy based on the parameters given from the 
    \"./aws-iam-permissions/create_group.json\" file """
    
            

if __name__ == "__main__":
    group = read_permission_documents()
